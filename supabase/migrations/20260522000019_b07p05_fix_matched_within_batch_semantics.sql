-- B07·P05 — Fix-up: matched_within_batch reflects upload identity, not detection path
--
-- Original semantics: matched_within_batch=true only when the prior NEW
-- classification in the same dedup_run was the match path. In practice
-- unreachable: the first NEW inserts into transactions immediately, so the
-- strict pass against transactions catches the second occurrence first.
--
-- Fixed semantics: matched_within_batch = TRUE when the matched transaction's
-- statement_upload_id equals the current run's statement_upload_id (the
-- duplicate is within *this* upload, not a cross-statement duplicate). This
-- matches the downstream bookkeeping question: "is this dup of a row in this
-- upload, or of a previously-imported transaction?"
...
BEGIN
  ...
  IF FOUND THEN
    RETURN jsonb_build_object('ok', true, 'idempotent_replay', true,
      ...
      'matched_within_batch', v_existing_class.matched_within_batch);
  END IF;
  ...
  -- Strict pass (a): same source_row_hash exists in transactions
  ...
  IF FOUND THEN
    ...
    v_matched_tx_upload_id := v_match_exact.statement_upload_id;
    v_audit_action := 'TRANSACTION_DEDUP_EXACT_DUPLICATE';
  ELSE
    -- Strict pass (b): rare case — prior in-batch row was itself a duplicate
    -- (no transactions row inserted), but same source_row_hash. Catches the
    -- duplicate-of-a-duplicate within the batch.
    ...
    IF FOUND THEN
      ...
      v_matched_tx_upload_id := v_run.statement_upload_id; -- by definition
      v_audit_action := 'TRANSACTION_DEDUP_EXACT_DUPLICATE';
    ELSE
      ...
      IF FOUND THEN
        ...
        v_matched_tx_upload_id := v_match_soft.statement_upload_id;
  ...
  -- Within-batch flag: matched transaction belongs to the same upload as this run.
  IF v_matched_tx_upload_id IS NOT NULL AND v_matched_tx_upload_id = v_run.statement_upload_id THEN
    v_within_batch := true;
  END IF;
