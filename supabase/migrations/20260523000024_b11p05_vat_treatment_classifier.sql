-- B11·P05 part 2 of 2 — VAT Treatment Classifier
-- =====================================================================
-- Rules-only, deterministic, total. Picks exactly one of the spec-canonical
-- 8 treatments per draft entry. AI never appears in the classification path.
--
-- 4 IMMUTABLE helpers + 1 SECURITY DEFINER classifier RPC.
--
-- Pipeline (first hit wins):
--   PRE-1 manual override → honor existing treatment + emit OVERRIDE audit
--   PRE-2 business not VAT-registered → OUT=NO_VAT, IN=OUTSIDE_SCOPE
--   PRE-3 counterparty country unresolved → UNKNOWN + review reason
--   OUT side (rules OUT-1..OUT-8 + OUT-residual)
--   IN  side (rules IN-1..IN-5 + IN-residual + IN-2-residual)
--   + tag-mismatch detection (keeps treatment, sets requires_accountant_review)
--
-- Persists treatment + review flag to draft_ledger_entries.
-- Emits LEDGER_VAT_TREATMENT_DECIDED / _UNKNOWN_RAISED / _HONORED_MANUAL_OVERRIDE
-- and _TAG_MISMATCH_DETECTED (subject_type=DRAFT_LEDGER_ENTRY, actor_kind=SYSTEM).
-- =====================================================================

BEGIN;

-- 1. EU member-state set (Stage-1 freeze) ---------------------------------
CREATE OR REPLACE FUNCTION public.is_eu_member_state(p_country char(2))
...
-- 2. Service/goods tag inference ------------------------------------------
CREATE OR REPLACE FUNCTION public.infer_service_or_goods(p_tag text)
RETURNS text LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE
    WHEN p_tag IS NULL THEN 'UNKNOWN'
    WHEN lower(p_tag) IN ('it_software','saas_subscription','professional_fees','marketing','subscriptions','consulting','service','digital_service') THEN 'SERVICE'
    WHEN lower(p_tag) IN ('office_supplies','inventory','goods','physical_goods_import','hardware','equipment') THEN 'GOODS'
    ELSE 'UNKNOWN'
  END;
$$;

-- 3. Exempt-category check (Stage-1 placeholder; sub-doc finalises) -------
CREATE OR REPLACE FUNCTION public.is_exempt_category(p_category text)
RETURNS boolean LANGUAGE sql IMMUTABLE AS $$
  SELECT lower(coalesce(p_category,'')) IN ('financial_services','education','healthcare','insurance');
$$;

-- 4. Outside-scope transaction-type check ---------------------------------
CREATE OR REPLACE FUNCTION public.is_outside_scope_transaction_type(p_type public.transaction_type_enum)
RETURNS boolean LANGUAGE sql IMMUTABLE AS $$
  SELECT p_type IN ('INTERNAL_TRANSFER','LOAN_OR_SHAREHOLDER_MOVEMENT','TAX_PAYMENT','BANK_FEE');
$$;


-- 5. classify_vat_treatment -----------------------------------------------
...
BEGIN
  ...
  IF v_parent_txn_id IS NOT NULL THEN
    SELECT direction, transaction_type, counterparty_country, counterparty_vat_number, user_tag
      INTO v_direction, v_txn_type, v_counterparty_country, v_counterparty_vat, v_txn_tag
  ...
  -- PRE-2: business not VAT-registered
  IF v_biz_vat_registered IS NULL OR v_biz_vat_registered = false THEN
    ...
    END IF;
  -- PRE-3: counterparty country unresolved
  ...
  -- Tag-mismatch detection (keeps treatment, sets review flag)
...
-- 6. Privileges -----------------------------------------------------------
REVOKE EXECUTE ON FUNCTION public.classify_vat_treatment(uuid, uuid, uuid, uuid, uuid, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.classify_vat_treatment(uuid, uuid, uuid, uuid, uuid, jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public.is_eu_member_state(char(2)) TO authenticated, service_role, anon;
GRANT EXECUTE ON FUNCTION public.infer_service_or_goods(text) TO authenticated, service_role, anon;
GRANT EXECUTE ON FUNCTION public.is_exempt_category(text) TO authenticated, service_role, anon;
GRANT EXECUTE ON FUNCTION public.is_outside_scope_transaction_type(public.transaction_type_enum) TO authenticated, service_role, anon;
