-- =============================================================================
-- BOOK-971 — link paired_run_id on event-created OUT/IN runs.
-- =============================================================================
-- consume_statement_upload_completed_event created the OUT_MONTHLY and IN_MONTHLY
-- runs with independent ids but never set paired_run_id, so event-triggered runs
-- (unlike create_paired_workflow_runs) were not linked to each other. Any code
-- relying on paired_run_id to resolve the sibling run broke for event runs.
--
-- Fix: pre-generate both ids; after inserting both, set each row's paired_run_id
-- to the other (post-insert UPDATE keeps it FK-safe regardless of insert order),
-- and only when both sides were actually created (either may be disabled by
-- business_workflow_config). Body otherwise verbatim.
-- =============================================================================

CREATE OR REPLACE FUNCTION public.consume_statement_upload_completed_event(p_event_id uuid, p_actor_user_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF NOT v_out_disabled THEN
    ...
    VALUES
      (v_out_id, v_evt.organization_id, v_evt.business_id, 'OUT_MONTHLY'::public.workflow_type_enum,
    ...
    v_created_runs := v_created_runs || ARRAY[v_out_id];
  ...
  IF NOT v_in_disabled THEN
    INSERT INTO public.workflow_runs
      (id, organization_id, business_id, workflow_type, principal_snapshot,
       ...
       summary_json, created_at, updated_at)
    VALUES
      (v_in_id, v_evt.organization_id, v_evt.business_id, 'IN_MONTHLY'::public.workflow_type_enum,
       jsonb_build_object('actor_user_id', v_evt.actor_user_id,
                          'event_id', v_evt.event_id,
                          'statement_upload_id', v_evt.statement_upload_id),
       v_evt.declared_period_start::timestamptz,
       ...
       'EVENT'::public.trigger_kind_enum, p_event_id::text, 'CREATED',
       '{}'::jsonb, clock_timestamp(), clock_timestamp());
    v_created_runs := v_created_runs || ARRAY[v_in_id];
    v_types_enabled := v_types_enabled || ARRAY['IN_MONTHLY'];
  END IF;

  -- BOOK-971: link the pair when both sides were created.
  IF NOT v_out_disabled AND NOT v_in_disabled THEN
    UPDATE public.workflow_runs SET paired_run_id = v_in_id  WHERE id = v_out_id;
    UPDATE public.workflow_runs SET paired_run_id = v_out_id WHERE id = v_in_id;
  END IF;
