-- =============================================================================
-- BOOK-967 — VAT classifier: assume domestic when counterparty country is
-- unresolved, instead of returning UNKNOWN.
-- =============================================================================
-- classify_vat_treatment is a deterministic rule engine (no AI). But a bank-only
-- import with no evidence leaves counterparty_country NULL, which hit the
-- PRE-3-COUNTRY_UNRESOLVED branch → vat_treatment=UNKNOWN for every entry → the
-- "VAT classifications complete" finalization gate failed (unclassified_count),
-- so a domestic business could never finalize a no-evidence month.
--
-- Fix: when counterparty_country can't be resolved, assume the business's own
-- country (most SME transactions are domestic) so the directional rules yield a
-- definite treatment, and flag the entry for accountant review with a clear
-- reason so a human confirms before the VAT return is finalized (now resolvable
-- via the BOOK-968 CONFIRM_LEDGER_ENTRY action). Body otherwise verbatim; the
-- PRE-3 UNKNOWN branch remains as a safety net for the (impossible) both-null case.
-- =============================================================================

CREATE OR REPLACE FUNCTION public.classify_vat_treatment(p_organization_id uuid, p_business_id uuid, p_draft_ledger_entry_id uuid, p_workflow_run_id uuid, p_actor_user_id uuid DEFAULT NULL::uuid, p_context jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- BOOK-967: assume the business's own country when the counterparty country
  -- could not be resolved, so the rules below pick a definite domestic treatment
  -- instead of UNKNOWN. Flag for accountant review so a human confirms.
  IF v_counterparty_country IS NULL AND v_biz_country IS NOT NULL THEN
    v_counterparty_country := v_biz_country;
    v_review := true;
    v_reason := 'Counterparty country could not be determined; assumed domestic ('
                || v_biz_country || ') — confirm before finalizing.';
  END IF;
