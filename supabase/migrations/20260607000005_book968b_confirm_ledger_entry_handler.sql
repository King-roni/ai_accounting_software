-- =============================================================================
-- BOOK-968 (part B) — handle CONFIRM_LEDGER_ENTRY + allow it for accountant review.
-- =============================================================================
-- New CONFIRM_LEDGER_ENTRY action: an accountant confirms a flagged draft ledger
-- entry. It resolves the review issue (terminal, like the other resolving
-- actions) AND clears requires_accountant_review on the linked draft entry so a
-- rescan won't re-raise it. Added to ledger.requires_accountant_review's allowed
-- actions. apply_resolution_action body is otherwise verbatim.
-- =============================================================================
...
BEGIN
  ...
  CASE p_action
    ...
    WHEN 'CONFIRM_LEDGER_ENTRY'::public.resolution_action_kind_enum THEN
      -- BOOK-968: accountant confirms a flagged draft ledger entry. Resolves the
      -- issue (terminal) and clears the entry flag so a rescan won't re-raise it.
      v_downstream := 'draft_ledger_entries.requires_accountant_review CLEAR';
      IF v_issue.draft_ledger_entry_id IS NOT NULL THEN
        UPDATE public.draft_ledger_entries
           SET requires_accountant_review = false,
               accountant_review_reason   = NULL
         WHERE id = v_issue.draft_ledger_entry_id;
      END IF;
...
-- The registry CHECK constraint hardcodes the valid action list; extend it.
ALTER TABLE public.issue_type_registry DROP CONSTRAINT issue_type_registry_actions_enum_valid_chk;
ALTER TABLE public.issue_type_registry ADD CONSTRAINT issue_type_registry_actions_enum_valid_chk
  CHECK (allowed_resolution_actions <@ ARRAY[
    'UPLOAD_DOCUMENT','CONFIRM_MATCH','REJECT_MATCH','CHANGE_TAG','CHANGE_TRANSACTION_TYPE',
    'MARK_AS_INTERNAL_TRANSFER','MARK_AS_BANK_FEE','MARK_AS_NON_DEDUCTIBLE','MARK_AS_NO_INVOICE_AVAILABLE',
    'ADD_EXPLANATION_NOTE','SEND_TO_ACCOUNTANT_REVIEW','IGNORE_WITH_REASON','RERUN_SCAN_AFTER_CHANGE',
    'CONFIRM_CLASSIFICATION','CONFIRM_LEDGER_ENTRY']::text[]);

-- Allow the new action on accountant-review issues.
UPDATE public.issue_type_registry
   SET allowed_resolution_actions = array_append(allowed_resolution_actions, 'CONFIRM_LEDGER_ENTRY')
 WHERE issue_type = 'ledger.requires_accountant_review'
   AND NOT ('CONFIRM_LEDGER_ENTRY' = ANY(allowed_resolution_actions));
