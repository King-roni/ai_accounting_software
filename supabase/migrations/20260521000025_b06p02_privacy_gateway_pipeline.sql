-- B06·P02 — Privacy Gateway Pipeline (migration 2 of 2)
--
-- The chokepoint every AI call passes through. Two-phase because SQL can't
-- call out to HTTP: application code orchestrates begin → external dispatch → finalize.
-- P05/P06 will wire real Anthropic / local-LLM dispatch on top of this contract.
--
-- Builds:
--   1. ai_gateway_invocation_status_enum + ai_gateway_result_variant_enum
--   2. ai_gateway_invocations table (one row per gateway invocation)
--   3. ai_gateway_minimize_payload() — drops keys not in schema.properties
--   4. ai_gateway_invoke_begin() — validate input / minimize / route, returns PREPARED
--   5. ai_gateway_invoke_finalize() — validate output, returns terminal AIResult variant
--
-- Audit events emitted (registered in canonical taxonomy doc as part of this phase):
--   AI_GATEWAY_INVOKED                — every invocation that reaches model dispatch
--   AI_GATEWAY_VALIDATION_FAILED      — input fails tool's input_schema
--   AI_GATEWAY_RESPONSE_INVALID       — model output fails tool's output_schema
-- (AI_TIER_BLOCKED on opt-out is emitted by route_ai_call and not re-emitted here.)
--
-- Migration 1 added AI_GATEWAY_INVOCATION to audit.subject_type_enum.
...
CREATE TYPE public.ai_gateway_invocation_status_enum AS ENUM (
  ...
  'COMPLETED_REDACTION_REJECTED'
);
COMMENT ON TYPE public.ai_gateway_invocation_status_enum IS
  'Lifecycle status of a gateway invocation row. PREPARED is the post-begin in-flight state; every COMPLETED_* value maps 1:1 to a spec AIResult variant.';
...
COMMENT ON TYPE public.ai_gateway_result_variant_enum IS
  'Canonical AIResult variant returned by the gateway (B06·P02 spec §AIResult variants). Stored on the invocation row when finalized and echoed in the envelope.';

-- ============================================================================
-- 2. ai_gateway_invocations
-- ============================================================================
...
COMMENT ON TABLE public.ai_gateway_invocations IS
  'One row per AI gateway invocation. Created by ai_gateway_invoke_begin (status=PREPARED) and updated by ai_gateway_invoke_finalize (status=COMPLETED_*). Each terminal row mirrors a spec AIResult variant. Rows with status=COMPLETED_SCHEMA_VIOLATION_INPUT / COMPLETED_TIER_BLOCKED are inserted directly in begin (no model dispatch occurred).';
...
-- ============================================================================
-- 3. ai_gateway_minimize_payload
-- ============================================================================
CREATE OR REPLACE FUNCTION public.ai_gateway_minimize_payload(p_schema jsonb, p_input jsonb)
RETURNS jsonb
LANGUAGE plpgsql
IMMUTABLE
PARALLEL SAFE
...
BEGIN
  ...
  IF v_props IS NULL OR jsonb_typeof(v_props) <> 'object' THEN
    -- schema declares no properties → no allowlist available; pass through unchanged
    RETURN p_input;
  END IF;
  FOR v_key IN SELECT * FROM jsonb_object_keys(p_input) LOOP
    IF v_props ? v_key THEN
      v_out := v_out || jsonb_build_object(v_key, p_input -> v_key);
    END IF;
  END LOOP;
  RETURN v_out;
END;
$function$;
COMMENT ON FUNCTION public.ai_gateway_minimize_payload(jsonb, jsonb) IS
  'Drops top-level keys not declared in schema.properties (allowlist-based per B06·P02 spec §Pipeline step 2). Returns input unchanged when schema.properties is absent (degenerate schema) or input is not an object.';

-- ============================================================================
-- 4. ai_gateway_invoke_begin
-- ============================================================================
...
BEGIN
  ...
  IF v_tool.ai_tier = 'NONE'::public.ai_tier_enum THEN
    -- Tier 1 must not reach the gateway (spec §Pipeline step 5)
    RETURN jsonb_build_object(
      'ok', false, 'decision', 'ERROR',
  ...
  -- Step 2: payload minimization
  v_minimized := public.ai_gateway_minimize_payload(v_tool.input_schema, p_input);

  -- Step 3: redaction policy — DEFERRED to B06·P03. No-op placeholder for now.
  ...
  IF v_route_dec = 'BLOCK' THEN
    ...
    RETURN jsonb_build_object(
      ...
      'audit_event_id', v_route->>'audit_event_id'  -- AI_TIER_BLOCKED row
    );
  ELSIF v_route_dec <> 'ALLOW' THEN
    -- ERROR or unexpected. Surface as MODEL_ERROR-style error (no row created).
    RETURN jsonb_build_object(
      'ok', false, 'decision', 'ERROR',
  ...
  -- Happy path: row inserted with status=PREPARED; caller dispatches; AI_GATEWAY_INVOKED emitted.
  INSERT INTO public.ai_gateway_invocations (
...
COMMENT ON FUNCTION public.ai_gateway_invoke_begin(text, uuid, jsonb, uuid, text, uuid) IS
  'AI gateway chokepoint (begin phase). Runs spec §Pipeline steps 1-4: input-schema validation, payload minimization, redaction-policy placeholder (P03), routing. Returns PREPARED envelope on success (caller dispatches externally then calls finalize) or an early-terminal envelope on validation failure / tier block. Emits AI_GATEWAY_INVOKED on prepare, AI_GATEWAY_VALIDATION_FAILED on input-schema failure. Tier 1 (no-AI) tools must not reach this entrypoint and are refused with TIER_1_BYPASS_REQUIRED.';

REVOKE EXECUTE ON FUNCTION public.ai_gateway_invoke_begin(text, uuid, jsonb, uuid, text, uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.ai_gateway_invoke_begin(text, uuid, jsonb, uuid, text, uuid)
  TO service_role;

-- ============================================================================
-- 5. ai_gateway_invoke_finalize
-- ============================================================================
...
BEGIN
  ...
  -- Usage logging (AI_USAGE_RECORDED) is deferred to B06·P07; no audit emitted here on success.
  RETURN jsonb_build_object(
    'ok',               true,
    'invocation_id',    p_invocation_id,
...
COMMENT ON FUNCTION public.ai_gateway_invoke_finalize(uuid, text, jsonb, jsonb) IS
  'AI gateway finalize phase. Validates the dispatched model response against the tool''s output_schema, transitions the invocation row from PREPARED to a COMPLETED_* terminal state, and returns the typed AIResult envelope. Emits AI_GATEWAY_RESPONSE_INVALID on output-schema failure. dispatch_status=ERROR shortcuts to MODEL_ERROR (caller decides retry per B03·P08). Rejects unknown invocation_id (INVOCATION_NOT_FOUND) and already-finalized rows (ALREADY_FINALIZED). Usage logging (AI_USAGE_RECORDED) is deferred to B06·P07.';

REVOKE EXECUTE ON FUNCTION public.ai_gateway_invoke_finalize(uuid, text, jsonb, jsonb)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.ai_gateway_invoke_finalize(uuid, text, jsonb, jsonb)
  TO service_role;
