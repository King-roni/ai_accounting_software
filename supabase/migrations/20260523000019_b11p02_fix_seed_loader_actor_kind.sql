-- B11·P02 follow-up — Mitigation A actor_kind constraint enforcement.
-- The seed loader's audit.emit_audit calls were passing p_actor_user_id while
-- using actor_kind='SYSTEM' — that violates the audit_events_actor_kind_chk
-- constraint (SYSTEM actor cannot also carry actor_user_id). Drop the
-- actor_user_id on the audit emits; the user_id still lives on
-- chart_of_accounts_mapping_versions.created_by and in the after_state
-- payload as 'initiating_user_id' for traceability.
--
-- The function body matches 20260523000018 except for the
-- p_actor_user_id:=NULL fix on every emit_audit call.

-- (Migration body intentionally identical to the patched version applied to
-- the DB; reproduce it here for forward-only replay.)
